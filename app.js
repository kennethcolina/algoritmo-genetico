(function($) {
    ...
    modulo.controller('AppCtrl', function($scope) {
        ...
        $scope.taxaDeCrossover = 70;
        $scope.taxaDeMutacao = 1;
        $scope.maxDeGeracoes = 200;
        $scope.tamanhoPopulacao = 30;
        $scope.geracoesFinal = [];
        $scope.geracoesDataGrafico = [];
        $scope.geracoesLabelsGrafico = [];
        var qtGeracoes = 0;
        $scope.funcao = 0;
        $scope.funcaoAtual = 0;
        $scope.objetivo = "maximizar";
        ...
        $scope.algoritmoGenetico = function algoritmoGenetico() {
            ...
            $scope.geracoesFinal = [];
            var qtGeracoes = 0;
            $scope.funcaoAtual = $scope.funcao;
            ...
            do {
                ...
                if (criterioDeParada() == true) gerarGrafico(geracoes);
            
            } while (criterioDeParada() == false);

            function gerarGrafico(geracoes) {
               ...
               $scope.labels = [];
                ...
                $scope.geracoesFinal.forEach(function(g) {
                    g.populacao.forEach(function(i) {
                        $scope.dataAux.push(i.aptidao);
                        $scope.labels.push(i.decimal.toString());
                    }, this);
                }, this);
